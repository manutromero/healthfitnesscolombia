;(function($) {
	...
	PPOffcanvasCart = function( $scope ) {
		
		this.node                 = $scope;
		this.wrap                 = $scope.find('.pp-offcanvas-cart-container');
		this.content              = $scope.find('.pp-offcanvas-cart');
	...
	PPOffcanvasCart.prototype = {
		...
		init: function () {
			...
			if ( this.wrap.find('.pp-offcanvas-cart').length > 0 ) {
                ...
                $('body').prepend( this.wrap.find('.pp-offcanvas-cart') );
			}
		...
		closeClick: function() {
			...
			$(document).on('click', function(e) {
				if ( $(e.target).is('.pp-offcanvas-cart') || $(e.target).parents('.pp-offcanvas-cart').length > 0 || $(e.target).is('.pp-offcanvas-toggle') || $(e.target).parents('.pp-offcanvas-toggle').length > 0 || $(e.target).is($trigger) || $(e.target).parents($trigger).length > 0 ) {
					return;
				} else {
					self.close();
				}
